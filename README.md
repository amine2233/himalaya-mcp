...
| `compose_email` | `to`, `subject`, `body`, `attachments?`, `account?` | `template write` → template |
| `draft_reply` | `id`, `body?`, `reply_all?`, `folder?`, `account?` | `template reply <id>` → template |
| `send_email` | `template`, `attachments?`, `confirm`, `account?` | `template send <template>` |
| `send_template` | `template` \| `template_file`, `attachments?`, `confirm`, `account?` | `template send <template>` |
...
- `compose_email` / `draft_reply` produce a **template** and never send; feed it to `send_email`
  or `send_template`, which refuse unless `confirm=true` (sending is irreversible). Attachments are
  expressed as himalaya MML `<#part>` directives. See [Messages vs templates](#messages-vs-templates).
...
This server works entirely with **templates**: `compose_email` and `draft_reply` emit a template
(they use `template write`/`template reply` — *not* `message write`, which would open an editor), and
`send_email` / `send_template` send it with `template send`. Sending a template through
`message send` fails, because the MML/template isn't a valid raw message.

#### Sending with templates (`send_template`)

Use `send_template` to send a template you author yourself (or one produced by `compose_email` /
`draft_reply`). Provide it **inline** or **from a file**, and — because sending can't be undone —
set `confirm: true`.
...
- The full compose→send flow: `compose_email` → review the returned template → `send_template`
  (or `send_email`) with `confirm: true`.
